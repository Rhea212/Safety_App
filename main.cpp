...
public:
    ...
    void show ()
    {
        ...
        while (it != credentials.end ())
        {
            cout << "userid: " << it->first << "\tpassword: " << it->second << endl;
            it++;
        }
    }
...
class RSA{
	private:
		...
		string registered_email="shivammishramalad@gmail.com";
	public:
		void values();
		void eea();
};
...
void otpSend(string registered_email){
	...
	do{
		...
		SendMail(0);
		// otpVerify();
		std::cout << "OTP is valid only for 5 min!!" << std::endl;
...
void symmetricKeyGen(string registered_email){
	printf("Symmetric key will be emailed on registred email address\n");
...
void RSA::eea()
{
	...
	// cout<<"\ne="<<e<<",d="<<d;
	cout<<"Asymmetric key will be emailed on registred email address\n";
...
void encryption(string registered_email){
	...
	printf("\nEnter approximate size of message\n*note: size can be greater than message but not less.\n-->");
	scanf("%d",&size);
	fflush(stdin);
	printf("\nEnter message\n-->");
	char message[size];
	...
	cout<<"\nEnter email address whom you want to send cipher text\n*note: only your and given email address will able to decrypt the text\n-->";
	cin>>receiver_email;
	...
	cout<<"\nEncrypted text will be emailed on registred email address\n";
...
void decryption(string registered_email){
	...
	printf("\nEnter approximate size of cipher\n*note: size can be greater than cipher but not less.\n-->");
	scanf("%d",&size_text);
	fflush(stdin);
	...
	string sender_email;
	cout<<"\nEnter email address whom you received cipher text\n*note: only receiver and sender email address will able to decrypt the text\n-->";
	cin>>sender_email;
	...
	cout<<"\nDecrypted text will be emailed on registred email address\n";
...
int main(){
    ...
    home:   while(true){
    system("cls");
    cout<<"Welcome to the safety app"<<endl;
    cout<<"1. Log-in"<<endl;
    cout<<"2. Sign-up"<<endl;
    cout<<"3. Exit"<<endl;
    cin>>choice;
    switch (choice) {
        case 1:
            ...
            if(m.passVerify(email, pass)){
                //goto features;  //giving the access to the features and otp wala stuff;
            }
            else{
